...
if( $act == 'idx' ){
    ...
    // }
        ...
        if($nod == 'save_order_service'){
            $lItems = $main->post('lItems');


            $general_service->set('lItems',$lItems);
            // echo $lItems ;
            // exit();
            $data = json_decode($lItems, true);
            // print_r($data);
            // exit();
            foreach($data as $item){
                ...
                // echo($item['price']);
                // exit();
                $general_service->set('booking_id',$item['booking_id']);
                $general_service->set('service_id',$item['service_id']);
                $general_service->set('quantity',$item['quantity']);
                $general_service->set('note',$item['note']);
                $general_service->set('booking_status_id',8); // status
                $general_service->set('total',$item['into_money']);
